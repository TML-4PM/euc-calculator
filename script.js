...
function parseEmailAndFillForm() {
  ...
  // Reset all product checkboxes and quantities
  productList.forEach(product => {
    const inputElem = document.querySelector(`input[data-product-id="${product.id}"]`);
    if (inputElem) inputElem.value = 0;
    const checkboxElem = document.getElementById(product.id);
    if (checkboxElem) checkboxElem.checked = false;
  });

  let unknownProducts = [];
  productList.forEach(product => {
    // Build a regex for the product name followed by an optional quantity number
    const regex = new RegExp(escapeRegExp(product.name) + '\\s*(\\d+)', "i");
    ...
    if (match) {
      ...
      // If product name is mentioned without a quantity, add to unknown products
      const unknownRegex = new RegExp(escapeRegExp(product.name), "i");
      if (unknownRegex.test(emailContent)) {
        unknownProducts.push(product.name);
      }
    }
  });

  // Append unknown products to the additional notes field
...
function calculateAndDisplay(event) {
  ...
  const baseHours = selectedProducts.length; // Assume 1 hour per product
  ...
  const productsCost = selectedProducts.reduce((sum, product) => sum + product.price * product.quantity, 0);
  const totalCost = productsCost + adjustedHours * 100; // Assume $100 per adjusted hour

  // Display totals with "USD $" prefix for consistency
  ...
  // Generate Barcode and QR Code with unique IDs
  const uniqueIdBarcode = `OW-${Math.random().toString(36).substr(2, 9)}`;
  const uniqueIdQRCode = `OW-${Math.random().toString(36).substr(2, 9)}`;
  generateBarcode(uniqueIdBarcode);
  generateQRCode(uniqueIdQRCode);

  document.getElementById("results").style.display = "block";
}

// Generate Barcode (requires JsBarcode library)
...
function suggestDates(event) {
  ...
  // Generate a random date within the next 10 days
  const randomDate = () => {
    const date = new Date(today.getTime() + Math.random() * 10 * 24 * 60 * 60 * 1000);
    return date.toISOString().split("T")[0];
  };

  const suggestedDate = randomDate();
  const suggestedTimeOptions = ["9:00 AM", "1:00 PM", "3:00 PM"];
  const suggestedTime = suggestedTimeOptions[Math.floor(Math.random() * suggestedTimeOptions.length)];
...
function confirmAppointment(event) {
  ...
  const totalCost = parseFloat(totalCostText.replace("USD $", ""));
...
// Send Email Confirmation (for the separate "Send Email" button)
function sendEmailConfirmation() {
  ...
  if (!installationDate) {
    showModal("Please select an installation date before sending email.", "Error");
    return;
  }
  ...
  sendEmail("troy.latter@unisys.com", emailSubject, emailBody);
  showModal("Email sent successfully!", "Confirmation");
}
...
function printToPDF() {
  ...
  productList
    ...
    .forEach(product => {
      ...
      const quantity = parseInt(quantityInput ? quantityInput.value : 0);
      doc.text(
        `${product.name} (${product.category}) - Quantity: ${quantity}, Price: $${product.price.toFixed(2)}, Total: $${(product.price * quantity).toFixed(2)}`,
        10,
        y += 10
      );
    });
  ...
  // Add Logos – wait for images to load before saving the PDF
  const logos = [
...
// Send Email (opens the mail client)
